...
std::string SHA256(const std::array<uint8_t, Size>& ptr) {
...
template <std::size_t Bytes>
void PrintBits(const std::array<uint8_t, Bytes>& arr) {
...
int main() {
    ...
    /* our parent hash, we are going to compare every hash with this one */
    std::array<uint8_t, bytes> parent{0};

    /* the input */
    std::vector<std::array<uint8_t, bytes>> data(GenerateData<bytes>(hashes_count));
